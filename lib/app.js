...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    console.log('look at me im an open port');
    if(request.path === '/' && request.method === 'GET'){
      ...
      socket.end(createResponse({ body:'<h1>red</h1>', status: '200 OK', contentType: 'text/plain' }));
    } else if(request.path === '/green' && request.method === 'GET') {
      socket.end(createResponse({ body:'<h1>green</h1>', status: '200 OK', contentType: 'text/plain' }));
    }  else if(request.path === '/blue' && request.method === 'GET') {
      socket.end(createResponse({ body:'<h1>blue</h1>', status: '200 OK', contentType: 'text/plain' }));
    }
